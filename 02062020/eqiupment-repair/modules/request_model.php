 ...
 class Model_Request{
 	...
 	function insert_data($fields,$id){
		...
		/////////////end copy file/////////////	
			...
			if($num_check_mail>='1' && $record_check_mail[FManagerID]!='25'){
				$sql_mail = "SELECT
 	...
 	function close_request_all($fields){
		  ...
		  $sql_check_mail = "SELECT
							...
							DATE_FORMAT(mtrequest_db.tbl_request.FFinishDate, '%d/%m/%Y') AS FFinishDate1,
							mtrequest_db.tbl_request.FFinishDate,
							tbl_employee_m.email_company,
							pis_db.tbl_section.sec_nameThai,
							tbl_employee_e.emp_name
							FROM
							mtrequest_db.tbl_request
							LEFT JOIN general_db.tbl_manager ON mtrequest_db.tbl_request.FManagerID = general_db.tbl_manager.FManagerID
							LEFT JOIN pis_db.tbl_employee AS tbl_employee_m ON general_db.tbl_manager.emp_code_full = pis_db.tbl_employee_m.emp_code_full
							LEFT JOIN pis_db.tbl_section ON mtrequest_db.tbl_request.FSectionID = pis_db.tbl_section.sec_id
							LEFT JOIN pis_db.tbl_employee AS tbl_employee_e ON mtrequest_db.tbl_request.FReqID = pis_db.tbl_employee_e.emp_id
					WHERE mtrequest_db.tbl_request.FRequestID = '".$fields['FRequestID']."' 
					AND (pis_db.tbl_employee_m.email_company!='' OR pis_db.tbl_employee_m.email_company IS NOT NULL)";
			$results_check_mail = mysql_query($sql_check_mail);	
			$num_check_mail = mysql_num_rows($results_check_mail);		
			$record_check_mail = mysql_fetch_array($results_check_mail);
			if($num_check_mail>='1'){
				...
				//require 'PHPMailer-master/PHPMailerAutoload.php';
				require '../../lib/PHPMailer-master/PHPMailerAutoload.php';
				$link_ckick="<a href='http://10.2.1.167/TNBSystems'>http://10.2.1.167/TNBSystems</a>";	
				$mail = new PHPMailer();
				$mail->CharSet = "UTF-8";
				$mail->ContentType = "text/html";
				$mail->isSMTP();// Set mailer to use SMTP
				$mail->Host = $record_mail[set_mailer_host];  
				$mail->From = $record_mail[set_mailer_from];
				$mail->FromName = iconv("tis-620","utf-8","ระบบ  MT REQUEST");
				$mail->AddAddress($record_check_mail[email_company], 1);
				$mail->AddAddress('isdp@toyotanont.com', 2);
				    $mail->AddAddress('Jiranan_pom@toyotanont.com', 3);
				$mail->WordWrap = 50;	// Set word wrap to 50 characters
				$mail->isHTML(true);// Set email format to HTML
				$mail->Subject = iconv("tis-620","utf-8","ระบบ  MT REQUEST เสร็จสิ้น"); 
				$mail->Body = iconv('tis-620','utf-8','เรียน คุณ ')
				.iconv('tis-620','utf-8',$record_check_mail[FManagerName])
				.iconv('tis-620','utf-8','<br/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;เนื่องจากทาง')
				.iconv('tis-620','utf-8',$record_check_mail[sec_nameThai])
				.iconv('tis-620','utf-8','&nbsp;&nbsp;ได้มีการแจ้งซ่อมระบบงานผ่านทาง ระบบ  MT REQUEST')
				.iconv('tis-620','utf-8','&nbsp;ใบ MT REQUEST หมายเลข ')
				.iconv('tis-620','utf-8',$record_check_mail[FReqNo])
				.iconv('tis-620','utf-8','<br/>')
				.iconv('tis-620','utf-8','&nbsp;โดยคุณ')
				.iconv('tis-620','utf-8',$record_check_mail[emp_name])
				.iconv('tis-620','utf-8','&nbsp;(ผู้แจ้ง)')
				.iconv('tis-620','utf-8','<br/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<b>รายละเอียดการแจ้ง</b>&nbsp;&nbsp;')
				.iconv('tis-620','utf-8',$record_check_mail[FDetail])
				.iconv('tis-620','utf-8','<br/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;ทางแผนกซ่อมบำรุงได้ดำเนินการซ่อม/ปรับปรุง/แก้ไข เรียบร้อยแล้ว  เมื่อวันที่&nbsp;')
				.iconv('tis-620','utf-8',$record_check_mail[FFinishDate1])
				."<br/><br/>"
				.iconv('tis-620','utf-8','&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;จึงเรียนเชิญ')
				.iconv('tis-620','utf-8','&nbsp;คุณ')
				.iconv('tis-620','utf-8',$record_check_mail[emp_name])
				.iconv('tis-620','utf-8','&nbsp;(ผู้แจ้ง)')
				.iconv('tis-620','utf-8',$record_check_mail[sec_nameThai])
				.iconv('tis-620','utf-8','&nbsp;ตรวจรับงานหมายเลข   MT REQUES ดังกล่าว  ผ่านระบบ  MT REQUEST ซึ่งจะปรากฎในส่วนของหน้า  Dashboard(คำร้องรอการตรวจรับงาน)')
				."<br/><br/>"
				//.iconv('tis-620','utf-8','สามารถดูรายละเอียดเพิ่มเติมได้ที่ ')."<br/>"
				//.iconv('tis-620','utf-8','สำหรับภายใน'). $link_ckick."<br/>"
				."..................................................................................................................................................................................<br />"
 	...
 	function get_data($id){
 		$dataArr = array();
 	 	$select_sql ="SELECT t1.* "
					...
					."LEFT JOIN general_db.tbl_worklate ON t1.id_worklate = general_db.tbl_worklate.id_worklate "
	...
	function get_data_list($params){
		...
		while($val=mysql_fetch_array($select_rst)){
			...
			$cell = array(
					...
					,"closejob_emp_date"=>iconv("TIS-620","UTF-8",$val['closejob_emp_date'])
					,"closejob_emp_detail"=>iconv("TIS-620","UTF-8",$val['closejob_emp_detail'])
					,"id_worklate"=>iconv("TIS-620","UTF-8",$val['id_worklate'])
			);
