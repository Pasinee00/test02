...
<body>
   <div class="dialog-panel condition-panel">
   		<div class="top-row">
   			<div class="left"></div>
   			<div class="center">
   				<span class="dialog-title">รายงานการส่งซ่อม / ส่ง claim</span>
   			</div>
   			<div class="right"></div>
   		</div> 
                	...
                	<tr>
   				   ...
   				   <tr>
   					...
   					<tr>
   						...
   						<td style="width:15%"><b>วันที่ส่ง :</b></td>
   						<td colspan="2" style="width:35%">
   							<input type="text" name="SRequestDate" id="SSendDate" style="width:80%;" value="">
   							-
   							<input type="text" name="ERequestDate" id="ESendDate" style="width:80%;" value="">
   						</td>
   					</tr>
   					<tr>
   						...
   						<td><b>ประเภท :</b></td>
   						<td colspan="2">
   							<input type="checkbox" name="FType_SR" id="FType_SR" value="SR">ส่งซ่อม
   							<input type="checkbox" name="FType_SC" id="FType_SC" value="SC">ส่ง Claim
   						</td>
   					</tr>
   					<tr>
   						<td><b>Suplier :</b></td>
...
$(document).ready(function() {
	 ...
	 $('#SSendDate').Zebra_DatePicker();
	 $('#ESendDate').Zebra_DatePicker();
...
function newWindow(){
	...
	params +="&SSendDate="+$('#SSendDate').val();
	params +="&ESendDate="+$('#ESendDate').val();
	params +="&SReceiveDate="+$('#SReceiveDate').val();
	params +="&EReceiveDate="+$('#EReceiveDate').val();
	params +="&FComClaimID="+$('#FComClaimID').val();
	if($('#FType_SR').is(':checked')==true)params +="&FType_SR=SR";
	if($('#FType_SC').is(':checked')==true)params +="&FType_SC=SC";
	newwindow=window.open('request-claim-report.page.tpl.php?'+params,
