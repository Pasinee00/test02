...
<div id="display-main" class="display-main">
    <div class="global-header">
      ...
      <div id="notify2"  style="display:none;position:absolute;right:5px;top:26px;">
	    <nav>
	      <ul>
      ...
      <div id="notify"  style="display:none;position:absolute;right:5px;top:26px;">
	    <nav>
	      <ul>
    ...
    <div class="login-panel" style="display:none">
    	<div id="login-form" class="login-form">
    		...
    		<div class="input-row" id="emp-panel" style="display:none">
    			...
    			<div class="text-input">
    				<div class="border-left"></div>
    			 	<input type="text" class="username" name="emp_code" id="emp-code">
    			 	<div class="border-right"></div>
    			</div>
    		</div>
...
<script>
				...
				.success(function(result) { 
					if(result>=1){
						...
						if(getCookie("login_type")=="D"){
							...
							$('#emp-panel').show();
							$('.loading-status').empty().html("Input Employee Code");
						}else{
						   setCookie("emp_id",getCookie("emp_id"),1);
						   loadSys(getCookie("u_id"),getCookie("emp_id"));
						}
					}else{
						$('.loading-status').empty().html("<font color=\"#990000\">Permission denied</font>");
					}
				});
		...
		function loadSys(userId,empId){
			...
			.success(function(result) { 
				...
				for(var i=0;i<result.length;i++){
					system = result[i];
					$('<div class="sys-list"><a href="javascript:void(0);" onClick="javascript:openSys(\''+system['id']+'\',\''+system['system']+'\',\''+system['url']+'\',\''+system['system']+'\',\''+empId+'\')">'+system['system']+'</a></div>').appendTo("div[id=sys-panel]");
				}
				$('.login-panel').hide();
				$('.login-overlay').hide();
			});
		...
		function getRequestNotify(){
			...
			.success(function(results) { 
				...
				setTimeout("getRequestNotify()",6000);
			});
		}
