      ...
      <div id="notify"  style="display:none;position:absolute;right:5px;top:10px;">
	    <nav>
	      <ul>
            ...
            <li><a href="#"  onclick="changePage(18,'templates/supports/request-holder.tpl.php?status=returnedit');">ผู้อนุมัติตีกลับ<span id="sreturn-mt" class="badge red">0</span></a></li>
		...
		function getRequestNotify(){
			...
			.success(function(results) { 
				...
				if(userType=="AM"){
					    ...
					    $('#sreturn-mt').empty().html(results['sreturn']);
				}else if(userType=="GM"){
					$('#snew').empty().html(results['snew']);
					$('#start').empty().html(results['start']);
